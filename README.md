# YOLOV8多模态目标检测
## 数据集：./datasets/RGB_IR_fuse_labels/RGB_IR_fuse_labels

训练时，要注意把rgb图像的文件夹重命名为images，把ir图像的文件夹重命名为image。

## 运行结果：

./runs/detect/val5-8里面的*_pred.jpg全部为测试的结果，区别是不同的置信度。也可以直接跑python test.py来查看结果，根据自己的需要修改置信度。
